...
非平凡 PR 必须由默认分支 `.github/workflows/codex-review-gate.yml` 读取 GitHub 当前事实，并把 `codex-review` commit status 写到 live PR head。review 事件通过 `.github/workflows/codex-review-signal.yml` 唤醒：signal 的 workflow/job 权限均为空，不 checkout、不消费 artifact/cache、不写 status、不 dispatch，只以严格 run-name 暴露 PR number、head SHA、event 与 action。gate 的 `workflow_run` resolver 验证 canonical signal id/path/blob、特权 gate 自身 workflow ref/SHA/blob，并把 run-name 与 live open `main` PR identity 交叉核对；resolver 不执行 source code、不消费 artifact/cache。review-signal route、通过只读 relevance resolver 的 issue-comment route 与唯一 final writer 使用相同且设置 `queue: max` 的 PR-number job-level concurrency group；只有 relevant comment 才进入 writer queue，route 在排队串行执行中先写 pending 再 dispatch，绝不写 final。GitHub 不保证 concurrency pending 的严格 FIFO 顺序。issue-comment 必须先满足 Codex Bot identity 或 maintainer 完整 head trigger 规则，无关评论保持零 status、零 dispatch。`edited` / `deleted` route 将服务端事件时间与 source run ID 贯穿 pending、dispatch 和 evaluator；删除时间来自当前 Actions run 的服务端 `created_at`。最终 writer 只复用精确绑定 event time/run ID 的 routed pending，不得以处理时间另写 destructive pending；`submitted` / `created` / `dismissed` 只失效旧结果。最终 writer 在 checkout 前绑定 base repository、`main`、base SHA、head repository 与 head SHA，只 checkout 精确 live base，以 Python isolated mode 启动 evaluator，并在 status 读取、pending 和最终写入前重验身份。该仓库自监督链提供 event-driven 快速失效与 heartbeat 最终修复，不宣称原子外部 custody；durable current-base epoch、old-base 隔离、默认分支 fanout、相同最终状态零写入、`workflow_dispatch` 禁令与整个 workflow/contract/checker 可信控制面保护保持不变。

signal run-name 同时绑定 actor login/type，特权 route 只接受 contract allowlist 中的 Codex `Bot`；普通 PR 作者或同名 human actor 的 review comment 不得写 pending 或进入最终 writer 队列。特权 review-trigger 入口及其完整本地 import / 动态 helper 闭包属于 immutable trusted controls，普通 PR 修改其中任一文件必须 fail closed。持有 `statuses: write` 的 job 引用 action 时必须固定完整 commit SHA。docs status publisher 的两个最终 live identity guard 都必须在身份漂移时调用受验证的 failure writer；bare nonzero exit 不能替代旧绿色状态的失效写入。
