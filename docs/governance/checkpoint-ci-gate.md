...
`loop/checkpoints` 也必须由默认分支/PR base 上的可信 workflow 和 verifier 读取 PR checkout 数据，不能执行 PR-controlled checker。状态写入必须与 PR 数据 checkout/验证分离：pending 和 result publisher job 可写 `statuses` 但不得 checkout，validation job 只读且不得写 status；两个 checkout 都必须关闭 credential persistence。Scope verifier 必须显式使用真实 PR base/head；`--base HEAD --head HEAD` 只允许未提交或 unborn 的本地工作树检查，不能成为 committed PR 的 Validation Claim。
