...
非平凡 PR 必须由默认分支 `.github/workflows/codex-review-gate.yml` 读取 GitHub 当前事实，并把 `codex-review` commit status 写到 live PR head。review 事件通过 `.github/workflows/codex-review-signal.yml` 唤醒：signal 的 workflow/job 权限均为空，不 checkout、不消费 artifact/cache、不写 status、不 dispatch，只以严格 run-name 暴露 PR number、head SHA、event/action、artifact ID、parent review ID、artifact head、payload 时间与 actor login/type。gate 的 `workflow_run` resolver 验证 canonical signal id/path/blob、特权 gate 自身 workflow ref/SHA/blob，并把完整 signal identity 与 live open `main` PR identity 交叉核对；相关 Codex signal 还必须按 ID 重读 artifact，通过 REST identity 与 GraphQL `PullRequestReview.updatedAt` 或 REST comment `updated_at` 绑定 live actor、head 和活动时间。resolver 不执行 source code、不消费 artifact/cache。普通 human review、明确绑定旧 head 的 Codex artifact 与无关 issue-comment 成功返回 `relevant=false`，不得进入 writer queue。
...
首次 root bootstrap 的 allowlist 必须从 trusted `required_paths` 与 `additional_required_checks` 纳入仓库自己的 index、有限 allowlist 中的 repo docs checker 和 product workflow，同时保持 Harness 控制面下界。产品源码、伪装成任意 `check_docs_*` 的脚本、绝对或 parent-relative 路径、symlink、非普通文件，以及 manifest 未绑定的 workflow 必须失败。
