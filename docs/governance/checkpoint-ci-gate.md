...
所有 routed review、inline 与 issue-comment action，包括 `created` / `edited` / `deleted` 以及 `submitted` / `dismissed`，都必须先写精确 pending：`Codex lease h=<10>;t=<time>;r=<run>;a=<artifact>;p=<parent>;base=<40>.`。该 lease tuple 是最终 writer 的所有权凭证；generic writer、older writer 或携带不同 tuple 的 writer 不得覆盖或终结现存 lease，只有 tuple 完全一致的 writer 才可复用 lease 并发布 final。review-signal route、通过只读 relevance resolver 的 issue-comment route 与唯一 final writer 使用相同且设置 `cancel-in-progress: false` 的 PR-number job-level concurrency group。GitHub 原生 concurrency 只保留一个 running 与一个 pending，后来的 pending 可替换较早 pending，且不保证 FIFO，因此 concurrency 不能替代 lease ownership；heartbeat 负责恢复被替换、丢失或延迟的事件。final writer 还必须证明未删除 artifact 唯一存在、来源可信、活动时间不早于 event time，或证明 deleted artifact 已不存在。lease 只在处理期间使旧结果失效，不得推进 durable base epoch cutoff 或隐藏无关 blocker。
