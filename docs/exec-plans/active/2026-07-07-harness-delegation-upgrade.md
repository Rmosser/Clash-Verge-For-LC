...
- Validation Claim: the complete current-round validation contract bound to canonical tree SHA-256 `6fb71c700666b126b1baa68c57af22006cec2f713af1ea4e29b7177e2da2219c` passed repository validators 29/29, canonical skill tests 190/190, rollout orchestration tests 411/411, and normalizer dry-run checks 29/29; the commit gate separately verifies the fixed SSH identity's signed attestation over the exact final report and dirty-tree digests before staging; the repository-specific dirty-tree digest is bound only by the signed external validation report and delivery manifest, avoiding Active Plan self-reference.
