...
- Validation Claim: the complete current-round validation contract bound to canonical tree SHA-256 `92ecd26342e01aab958170ca9a515a209e571f0d38962f9b8fce15231fd53949` passed repository validators 29/29, canonical skill tests 215/215, rollout orchestration tests 434/434, and normalizer dry-run checks 29/29; the commit gate separately verifies the fixed SSH identity's signed attestation over the exact final report and dirty-tree digests before staging; the repository-specific dirty-tree digest is bound only by the signed external validation report and delivery manifest, avoiding Active Plan self-reference.
