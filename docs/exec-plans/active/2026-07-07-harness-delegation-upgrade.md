...
- Harness regression test: `scripts/test_harness_current_head_findings.py`
...
- Change Claim: actual `git diff origin/main` plus current worktree paths include `.github/pull_request_template.md`, `.github/workflows/codex-review-gate.yml`, `.github/workflows/codex-review-heartbeat.yml`, `.github/workflows/codex-review-signal.yml`, `.github/workflows/docs-ci.yml`, `.harness/repo-contract.json`, `AGENTS.md`, `docs/doc-sync-rules.json`, `docs/exec-plans/active/.gitkeep`, `docs/exec-plans/active/2026-07-07-harness-delegation-upgrade.md`, `docs/exec-plans/active/20260530-runtime-hardening-recovery.md`, `docs/exec-plans/completed/.gitkeep`, `docs/exec-plans/completed/archived-before-harness-upgrade-20260530-runtime-hardening-recovery.md`, `docs/exec-plans/template.md`, `docs/governance/checkpoint-ci-gate.md`, `docs/index.md`, `scripts/check_codex_review.py`, `scripts/check_docs.py`, `scripts/check_loop_checkpoints.py`, `scripts/test_harness_current_head_findings.py`; the current repair binds the complete repository-specific docs workflow by a trusted SHA-256, validates the synthetic base/head merge tree, and enforces trusted lower bounds for required paths, checks, entrypoint links, exclusion globs, diff classes, and target invariants; Markdown and governance traversal rejects symlinks while including root README, delegated scopes cannot escape through absolute or parent-relative paths, failed default-branch validation still reconciles every open PR, standard Codex footers are accepted without accepting arbitrary tails, and unchanged status evidence causes zero writes after a second live identity check; governance text is reached through descriptor-rooted no-follow parent traversal, opened non-blocking, bounded to 1 MiB, and decoded only after regular-file verification; automatic current-head Codex artifacts, trusted head-bound triggers, exact control-file pins, closed diff classes, conservative docs classification, fail-closed shell fanout, and mandatory two-stage Active Plan archival are enforced.
- Validation Claim: the complete current-round validation contract bound to canonical tree SHA-256 `022468b2f6994946ea9f74af9975b0534247e56e5919107019e1cec6fefbc9a8` and complete Git-visible skill tree SHA-256 `493b3a3053d460d9af17926a836c800bfa10a36c710ecfca3824375c29d4d978` passed repository validators 29/29, canonical skill tests 308/308, rollout orchestration tests 462/462, and normalizer dry-run scanned 29/29 repositories with 0 changed paths; the commit gate separately verifies the fixed SSH identity's signed attestation over the exact final report and dirty-tree digests before staging; the repository-specific dirty-tree digest is bound only by the signed external validation report and delivery manifest, avoiding Active Plan self-reference.
...
| worker-v28-b | medium | comment 3609215965: unrelated deleted-comment invalidation | uncommitted worktree | focused evaluator regression; docs-all; checkpoint diff; JSON; Python compile; repository lint/test entrypoint | passed | main-Agent review, signed commit, and fresh current-head review |
| review-9 | high | independent findings: same-second blockers, trusted trigger provenance, symlink-safe preview and Scope reads, fixed worktree identity, signed dirty-tree evidence, temporary-index and signer preflight, idempotent checkpoint status, recoverable product reruns, and completed archive state | d75942aaf608d0ab0aa14aab1831d8a6cbe188bc | main-agent integration plus bounded subagent review batch | repair applied | main-agent validation |
| main-10 | high | stable signed evidence and closure recovery | working tree | signed current-round evidence contract + 308 skill tests + 462 rollout orchestration tests + 29 repository validators | passed | sign commit and request fresh current-head review |
