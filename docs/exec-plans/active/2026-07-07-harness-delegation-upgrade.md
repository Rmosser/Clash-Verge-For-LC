...
- Validation Claim: the complete current-round validation contract bound to canonical tree SHA-256 `5e769748cd22f147fdc0c20e040cc7a7a13a0864ae515e1d5eafb283ff39332b` passed repository validators 29/29, canonical skill tests 203/203, rollout orchestration tests 413/413, and normalizer dry-run checks 29/29; the commit gate separately verifies the fixed SSH identity's signed attestation over the exact final report and dirty-tree digests before staging; the repository-specific dirty-tree digest is bound only by the signed external validation report and delivery manifest, avoiding Active Plan self-reference.
