...
- Change Claim: actual `git diff origin/main...HEAD` plus worktree diff includes `.github/pull_request_template.md`, `.github/workflows/codex-review-gate.yml`, `.github/workflows/codex-review-heartbeat.yml`, `.github/workflows/docs-ci.yml`, `.harness/repo-contract.json`, `AGENTS.md`, `docs/doc-sync-rules.json`, `docs/exec-plans/active/.gitkeep`, `docs/exec-plans/active/2026-07-07-harness-delegation-upgrade.md`, `docs/exec-plans/completed/.gitkeep`, `docs/exec-plans/completed/archived-before-harness-upgrade-20260530-runtime-hardening-recovery.md`, `docs/exec-plans/template.md`, `docs/governance/checkpoint-ci-gate.md`, `scripts/check_codex_review.py`, `scripts/check_docs.py`, `scripts/check_loop_checkpoints.py`; the current repair fails closed on malformed review markers, bounds the canonical governance hash, rejects repository-escaping and site-root links, requires real Active Plan directories, treats fork PR content as data only, blocks incomplete COMMENTED results, applies repo-specific review-required diff classes, and binds V3 checkpoint status to live head repository/head/base evidence; required paths must remain real non-symlink files/directories of the trusted type, same-second review artifacts fail closed, every checkout uses v7, and platform proofs plus repair commits bind the live default-branch base SHA; the codex-review writer now binds open main PR base/head repository and SHA identity, while bootstrap control-plane review uses a hashed external fixture-only evaluator.
- Validation Claim: repo-native docs, real `origin/main` Scope, JSON, YAML, and diff checks passed on 2026-07-16; an exact canonical control hash and negative smoke reject any target execution added to the privileged validation job; canonical skill tests passed 85/85 and rollout orchestration tests passed 190/190; the real 30-repository normalizer dry-run reported zero pending paths; no repository-specific product command required relocation.
...
- Rework requested: completed - V3 live resolver/publisher head-repo/head/base binding, codex-review open-main PR/base binding, hashed external bootstrap evaluator, default-branch open-PR reconciliation, checkout v7, complete trusted-base history, real non-symlink required paths, plan-directory symlink rejection, inline/reference site-root link rejection, same-second artifact rejection, review-required doc classes, incomplete COMMENTED blocking, proof base identity, committer base-SHA checks, DMIT marker verification, owner-specific platform payload, strict protection, publish locking, and crash-idempotent review request findings are addressed
...
| review-3 | high | independent current-head findings: V3 trusted-base checkout, open-PR reconciliation, live head/base publisher binding, trusted manifest lower bound, required-path/plan-directory symlink, inline/reference site-root link, same-second and COMMENTED artifact, proof base identity, checkout v7, and sensitive-doc review policy | eac937b860eaf0de395a84747f5b71eb9b058b59 | canonical skill review plus repository review batch | repair applied | main-agent validation |
| main-4 | high | trusted-base reconciliation and current-head review repair | working tree | docs/Scope/JSON/YAML/Python/diff + preserved product checks + 85 skill tests + 190 rollout orchestration tests + 30-repo zero-diff preview | passed | sign commit and request fresh current-head review |
