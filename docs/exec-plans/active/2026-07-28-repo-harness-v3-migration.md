...
- Review rework round 41 shared-verifier: upheld raw-HTML-container, reserved evidence-context, and ambiguous plan-schema findings. Every workflow is inspected for the source-isolated `harness/evidence` name; legacy emitter deletion remains migratable. Active Plan headings and fields now use the documented canonical plain, top-level, one-line syntax, rejecting rendered-equivalent HTML, entity, task, quote, nested, formatted, or multiline variants instead of implementing a general Markdown renderer.
- Review rework round 40 documentation: upheld the escaped-comment finding; escaped openers remain visible and their literal closers are not misclassified as malformed real comments.
- Review rework round 41 documentation: upheld the path-kind finding. Only the two declared plan paths may be directories; inventory files and file-valued link targets must remain regular files.
